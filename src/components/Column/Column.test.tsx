...
describe('Column Component', () => {
    ...
    test('Column is rendered correctly', () => {
        render(<Provider store={store}><BrowserRouter><DragDropContext onDragEnd={jest.fn}><Column books={books} column={column} /></DragDropContext></BrowserRouter></Provider>);

        expect(screen.getByTestId("colTitle").textContent).toEqual(column.title);
        expect(screen.getByTestId('bookDraggable')).toHaveTextContent(/Learning Web Development with React and Bootstrap/i);
    });

    test('Home search is working correctly', () => {
        render(<Provider store={store}><BrowserRouter><DragDropContext onDragEnd={jest.fn}><Column books={books} column={column} /></DragDropContext></BrowserRouter></Provider>);
        
        
        act(() => {
            store.dispatch(booksActions.setSearchQuery('React'));
        });
        expect(screen.getByText('Learning Web Development with React and Bootstrap')).toBeInTheDocument();

        act(() => {
            store.dispatch(booksActions.setSearchQuery('xxx'));
        });
        expect(screen.queryByText('Learning Web Development with React and Bootstrap')).not.toBeInTheDocument();
